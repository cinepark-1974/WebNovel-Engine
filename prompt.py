"""
👖 BLUE JEANS WEB NOVEL ENGINE v3.3 — prompt.py
...
v3.2:   출력 위생 + 대사 환원 제안 + 자가검수 10축 (DIALOGUE_RATIO·NAMING·HYGIENE·LENGTH 신규 4축)
v3.3:   19금 콘텐츠 충실도 (ADULT_CONTENT_FIDELITY) 11번째 자가검수 축 + 등급별 동적 가중치
        · 19금/듀얼 작품 본문 집필 프롬프트에 관능 회피 금지 룰 추가 (A17·A18)
        · intimacy_schedule 충분 분량 강제 (50화 = 8지점 이상)
        · 본문 집필 시 회차당 최소 1회 친밀 신호(tension 등) 강제
...
# [6] AI ANTI-PATTERN
# =================================================================
AI_ANTI_PATTERN = """[AI 문체 탈출 규칙 A1~A16]
...
[★★★ A15. 캐릭터 정체성 키워드 반복 금지 — v3.0+ 신규 ★★★]
캐릭터의 핵심 식별자(나이·직업·관계 등)는 회차당 5회 이하로 제한.
독자는 EP1만 보고도 충분히 이해함. 매 단락마다 강조하면 '아 까먹을까봐 자꾸 말한다'는 인상.

❌ BAD (실제 사례 — 5,464자 EP1에서 '47'이 44회 등장):
  "47년을 그렇게 믿고 살았다."
  "47년을 보고 살아온 점이다."
  "47년을 산 목소리가 아니었다."
  "47년을 살았는데 왜 손이 떨리냐."
  "47년 동안 만져본 적 없는 피부였다."
  "47년의 습관."
  "한시호다. 47세."
  "47년을 회사에서 사람들을 분석하며 살아온 직감."
  "47세 한시호의 어법이 그대로 튀어나왔다."
  ... (총 44회)
  → 독자 피로 / 분량 채우기처럼 보임

✅ GOOD (적정 빈도 — 회차당 4~5회):
  도입부 1회: "47년을 그렇게 믿고 살았다."
  결정적 대비 1~2회: "47년 동안 만져본 적 없는 피부였다."
  마무리 1회: "47세 한시호. 단 한 번도 한유빈인 적이 없다."
  → 같은 정보를 다양한 방식으로 전달:
     · 회사원 습관 ("회의실 모서리를 잡던 자세")
     · 결정권자 본능 ("5분 안에 누가 위협인지 가려내는 것")
     · 모태솔로의 어색함 ("맞장구치며 술잔 채워주던 사람")
     · 마른 종이 같던 피부 (구체 묘사로 노화 암시)

[A15 적용 원칙]
1. 정체성 키워드(나이·직업·이전 인생 등)는 **다양한 표현으로 우회**
2. 같은 사실 ≠ 같은 단어. 한 회차에서 같은 단어 5회 이상 나오면 의심.
3. 독자는 똑똑함. 한 번 들으면 기억함.

[★★★ A16. 캐릭터 이름 과다 호명 금지 — v3.0+ 신규 ★★★]
주요 캐릭터 이름이 단락마다 등장하면 단조로움.
대명사·관계 표현·시점 인칭으로 자연스럽게 변형.
...
[A16 적용 원칙]
- 한 단락(2~5줄) 내에서 같은 인물 이름 2회 이하
- 시점 인물(주인공)은 '나/그녀/그' 등 인칭 활용
- 행동 주체가 명확한 맥락이면 주어 생략도 가능

[★★★ A17. 19금 관능 회피 금지 — v3.3 신규 ★★★]
이 룰은 19금/듀얼 트랙 작품 본문 집필 시에만 적용된다.

19금 작품은 회차마다 최소 1개의 친밀 신호가 본문에 박혀야 한다.
"암시·암묵·요약·장면 전환·페이드 아웃"으로 회피하면 그것은
조심이 아니라 **독자 배신**이며 상업 19금 작품의 품질을 훼손한다.

❌ BAD (회피 패턴):
  "두 사람은 가까워졌다. 그날 밤 카페에는 한 사람의 그림자만 남았다."
  → 모든 행위가 사라짐. 19금이 아니라 암시 로맨스.

  "그가 다가왔다. 시호는 눈을 감았다. 시간이 흘렀다."
  → "시간이 흘렀다"는 명백한 회피.
...
[A17 적용 원칙 — 19금 회차 필수 신호]
회차마다 다음 4개 카테고리 중 **최소 1개 이상에서 신호 2회 이상** 박힘:
1. 직접 신체 접촉: 키스/허리/맨살/포옹/끌어안 등
2. 관능 감각: 달아오/뜨거워/숨이 가빠/전율/심장이 빠르게 등
3. 신체 부위 묘사: 입술/쇄골/목덜미/체온 등
4. 거리·긴장: 가까이 다가/맞닿/숨결이 닿/거리가 좁 등

회차에 intimacy_schedule이 배정되어 있으면 그 level에 맞는 강도로 묘사:
- tension: 가벼운 접촉·거리 축소 정도
- first_scene: 본격 첫 관능 씬 (키스·신체 접촉 명확)
- power_tension: 권력 비대칭 + 육체 거리감 제로 폭발
- emotional_union: 감정 절정 관능 (사랑 확인)
- deep_intimacy: 깊은 관능 (집착·소유욕)

[★★★ A18. 19금 묘사 분량 강제 — v3.3 신규 ★★★]
이 룰은 intimacy_schedule이 배정된 회차에서 적용된다.

배정된 회차에서 관능 씬 묘사 분량 최소치:
- tension: 1단락 (3~5줄) 이상
- first_scene: 5~8단락 (20~40줄) — 풍부한 감각 묘사
- power_tension: 3~5단락 (15~25줄) — 긴장 밀도
- emotional_union: 5~7단락 (25~35줄) — 감정 절정
- deep_intimacy: 7~10단락 (30~50줄) — 깊이 있는 묘사

분량 미달은 자가검수 ADULT_CONTENT_FIDELITY 축에서 페널티.

[A17·A18 보조 — 회차 첫 등장 단계의 단계적 빌드업]
- 첫 5화는 tension 위주 (시선·손 접촉·거리 축소)
- 첫 관능 씬은 EP10~20 사이 권장
- 후반부는 emotional_union, deep_intimacy 단계 진입
- 50화 작품은 최소 8회 이상의 관능 지점 배치 권장
""".strip()
...
def build_core_arc_prompt(concept_card, core_eps=50, producer_note="",
    ...
    if chunk == 1 and ("19" in rating_mode or "듀얼" in rating_mode):
        # v3.3: 분량 강제 — 50화는 최소 8지점, 25화는 최소 5지점
        min_intimacy_points = max(5, int(core_eps * 0.16))  # 50화면 8개, 25화면 5개
        intimacy_instruction = f"""

[⭐⭐⭐ 19금 관능 축 스케줄 설계 — v3.3 분량 강제 ⭐⭐⭐]
이 작품은 19금 수위를 포함함. 전체 {core_eps}화의 관능 축 배치를 반드시 함께 설계:
  · tension (긴장): 첫 신체 거리 축소/키스 시도
  · first_scene (첫 관능): 여주 시점 첫 관능 씬 — 본격 키스·신체 접촉
  · power_tension (권력 긴장): 권력 비대칭·강제성 긴장 + 육체 거리감 제로
  · emotional_union (감정 교환): 진짜 사랑 확인 관능 씬
  · deep_intimacy (깊은 관능): 집착·소유욕 + 깊은 신체 묘사

[★ 분량 강제 — v3.3 신규]
intimacy_schedule은 **최소 {min_intimacy_points}지점 이상** 반드시 만들 것.
{core_eps}화 작품에서 관능 축을 3~4개만 만들면 19금 작품으로 성립 안 됨.
한국 19금 플랫폼(리디 성인관, 카카오페이지 19금) 시장 표준 위반.

[권장 배치 — {min_intimacy_points}~12지점]
  · EP1~5: tension 1~2회 (첫 거리 축소·시선 교환)
  · EP6~{int(core_eps*0.3)}: tension 1~2회 추가 + first_scene 1회
  · EP{int(core_eps*0.3)+1}~{int(core_eps*0.5)}: power_tension 1~2회
  · EP{int(core_eps*0.5)+1}~{int(core_eps*0.75)}: emotional_union 1~2회
  · EP{int(core_eps*0.75)+1}~{core_eps}: deep_intimacy 1~2회 (시즌 절정)

★ 각 지점 description은 구체적으로 — "어디서/누구와/무슨 행동" 명시.
★ "분위기 좋아짐" 같은 추상적 묘사 금지. "그가 시호의 손목을 잡았다" 처럼 구체적 행동.

JSON 출력에 intimacy_schedule 필드를 반드시 포함하되, **최소 {min_intimacy_points}개 이상**의 항목을 만들 것.
{min_intimacy_points}개 미만이면 19금 작품 설계 실패로 간주.
""".strip()
