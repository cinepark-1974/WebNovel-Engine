...
v3.1:   출력 위생 강화 (메타정보 블록 출력 금지 + 마크다운 escape 금지)
        - 회차 끝에 '첫 문장:/리캡 방식:/독자 질문:/클리프행어 유형:/
...
[★★★ v3.1 출력 위생 규칙 — 절대 지킬 것 ★★★]
1. 회차 끝에 어떤 메타정보 블록도 출력하지 말 것.
   다음 라벨로 시작하는 줄을 절대 본문에 포함시키지 말 것:
     - "첫 문장:" / "첫문장:"
     - "리캡 방식:" / "리캡:"
     - "독자 질문:" / "독자질문:"
     - "클리프행어 유형:" / "클리프행거 유형:"
     - "클리프행어 내용:" / "클리프행거 내용:"
     - "다음 회차 연결:" / "다음회차 연결:" / "다음 화 연결:"
   이런 메타정보는 작가의 기획 단계 자료일 뿐, 회차 본문에 절대 노출되어선 안 됨.
   회차는 마지막 클리프행어 문장으로 자연스럽게 끝나야 함.

2. 마크다운 escape 문자 출력 금지.
   - "\\*", "\\<", "\\>", "\\_" 같은 백슬래시 escape를 출력하지 말 것.
   - 별표 씬구분은 깨끗한 "***" 그대로 출력 (백슬래시 없이).
   - 꺾쇠 대신 작은따옴표 사용 ("'...'"). 일기·메모·속마음 모두 작은따옴표로 표기.

3. JSON/마크다운/주석/설명 금지. 순수 원고 텍스트만.
""".strip()
...
[★★★ v3.1 출력 위생 규칙 ★★★]
1. 19금 원본에 메타정보 블록(첫 문장:/리캡 방식:/독자 질문:/클리프행어 유형:/
   클리프행어 내용:/다음 회차 연결:)이 끝에 붙어 있어도, 15금 변환본에는 절대 옮기지 말 것.
   이 메타정보 줄들은 본문이 아니므로 통째로 제거하고 변환할 것.
2. 마크다운 escape 문자(\\*, \\<, \\>, \\_) 출력 금지.
3. 별표 씬구분은 깨끗한 "***", 꺾쇠 대신 작은따옴표("'...'") 사용.
""".strip()
